...
const LoginPage = () => {
  ...
  const [passwordValue, setPasswordValue] =
      useState('' || localStorage.getItem('password'));
  const [isSubmited, setIsSubmited] =
      useState(false);
  const [isChecked, setIsChecked] =
      useState(false || !!localStorage.getItem('checked'));
      ...
      <div className="container">
        ...
        <div className="form-block">
          <form className="login-form" autoComplete="off">
            ...
            ></input>
            {
              !emailValue && isSubmited &&
                <Banner text="Please enter your email" type="error" />}
            <PasswordInput passwordValue={passwordValue}
              passwordChangeHanlder={passwordChangeHanlder}/>
            {
              !passwordValue && isSubmited &&
                <Banner text="Please enter your password" type="error" />}
            <div className="form--container">
              <label htmlFor="1">
                <input
                  id="1"
                  className="input-chek"
                  type="checkbox"
                  checked={isChecked}
            ...
            {isSubmited &&
                passwordValue &&
                emailValue &&
                <Banner text="You are logged into app" type="success" />}
          </form>
        </div>
      </div>
      <SignInFooter/>
